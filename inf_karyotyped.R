...
dataname <- commandArgs(trailingOnly = TRUE)
  ...
  # Load either the original or the newly clustered data subsetted by sample and cell type/cluster.
...
annotations <- subset(donor_subset_sample@meta.data, select=c(status))
  ...
  # Add either the cell type name or cluster number to the annotation to make reading the results easier
...
inf_donor_subset_sample = CreateInfercnvObject(raw_counts_matrix = donor_subset_sample@assays$RNA$counts,
                                              ...
                                              # ref_group_names = c(paste0("reference_", cluster)))      # Choose either this row or the one below according to the data used.
