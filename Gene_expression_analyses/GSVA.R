...
fullMatrix_results <- left_join(fullMatrix_results,
                                distinct(subset(pseudo_obj@meta.data,
                                                select = c(new_SampleID, SampleID, Day_fixed, pool))))
...
gsva_long <- gsva_long %>%
  mutate(
    Pathways = case_when(
      Pathways %in% c("wnt_pathway","bmp_pathway", "fgf_pathway", "shh_pathway", "egf_pathway", "prc2_pathway", "ra_pathway") ~ paste(toupper(mapply(`[`, strsplit(Pathways, "_", fixed = TRUE), 1)), "signaling pathway", sep = " "),
